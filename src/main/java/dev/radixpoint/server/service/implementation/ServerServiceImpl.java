package dev.radixpoint.server.service.implementation;

import dev.radixpoint.server.model.Server;
import dev.radixpoint.server.repository.ServerRepo;
import dev.radixpoint.server.service.ServerService;
...
public class ServerServiceImpl implements ServerService {
    ...
    private String setServerImageUrl() {
        ...
        return ServletUriComponentsBuilder.fromCurrentContextPath()
                .path("/api/v1/server/image/" + imageNames[new Random().nextInt(imageNames.length - 1)])
                .toUriString();
    }
}
